...
; Date:				4-24-17
; 
; Purpose:			Test the performance of 2 different structs
;************************************************************************
...
main PROC
	...
	;test first
    ;mWrite "Testing Bad Alignment: "
    ;call test_field_access
    ;call test_bad_alignment
    
	;test second
    mWrite "Testing Good Alignment: "
    call test_field_access
    call test_good_alignment

	exit
main ENDP

;new procedures done to do the lab
;---------------------------------------------------
test_good_alignment PROC
;
; Test the performance of aligned structure by a measure of time.

; Execution time using Employee:  12030 milliseconds
;---------------------------------------------------
.data
ALIGN DWORD
startTime DWORD ?            ; align startTime
emp Employee <>            ; or: emp Employee <>
...
L1:    mov    emp.Years,5
    ...
    ret
test_good_alignment ENDP

;---------------------------------------------------
test_bad_alignment PROC
;
; Test the performance of misaligned structure by a measure of time.

; Execution time using EmployeeBad:  12215 milliseconds
;---------------------------------------------------
.data
ALIGN DWORD
startTimeBad DWORD ?            ; align startTime
empBad EmployeeBad <>            ; or: emp Employee <>

.code
    call    GetMSeconds        ; get starting time
    mov    startTimeBad,eax
    
    mov    ecx,0FFFFFFFFh        ; loop counter
L1:    mov    empBad.Years,5
    mov    empBad.SalaryHistory,35000
    loop    L1

    call    GetMSeconds        ; get starting time
    sub    eax,startTimeBad
    mWrite "Elapsed time: "
    call    WriteDec            ; display elapsed time
    call    Crlf

    ret
test_bad_alignment ENDP

;old struct made by the author
; ;---------------------------------------------------
; test_alignment PROC
; ;
